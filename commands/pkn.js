module.exports = {
    ...
    async execute(client, message) {
        message.author.send("Papier, kamień, nożyce?").then(async privateMsg => {
            ...
            message.mentions.users.forEach((value, key) => {
                if (!value.bot) {
                    ...
                    value.send('Papier, kamień, nożyce?').then(privateMentionsMsg => {
                        let collectorMention = new client.Discord.MessageCollector(privateMentionsMsg.channel, filter);
            ...
            let collector = new client.Discord.MessageCollector(privateMsg.channel, filter);
